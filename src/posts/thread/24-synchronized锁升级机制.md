...
```mermaid
flowchart TD
    A[线程尝试获取锁] --> B{检查Mark Word<br>是否为无锁状态}
    B -->|是| C[使用CAS设置为<br>偏向模式]
    C --> D{CAS操作<br>是否成功}
    D -->|是| E[获得偏向锁<br>执行同步代码]
    D -->|否| F[有竞争<br>撤销偏向锁]
    F --> G[升级为轻量级锁]
    B -->|否| H{检查线程ID<br>是否匹配}
    H -->|是| E
    H -->|否| F
```
...
```mermaid
flowchart TD
    A[线程尝试获取锁] --> B[在栈帧中创建<br>锁记录Lock Record]
    B --> C[复制对象头Mark Word<br>到锁记录]
    C --> D[使用CAS替换对象头<br>指向锁记录]
    D --> E{CAS操作<br>是否成功}
    E -->|是| F[获得轻量级锁<br>执行同步代码]
    E -->|否| G[自旋尝试获取锁]
    G --> H{自旋是否成功}
    H -->|是| F
    H -->|否| I[升级为重量级锁]
...
```mermaid
flowchart TD
    subgraph Monitor对象结构
    Owner[Owner<br>当前持有锁的线程] --> Entry[Entry Set<br>等待获取锁的线程队列]
    Owner --> Wait[Wait Set<br>调用wait的线程队列]
    end
    
    subgraph 线程竞争过程
    T1[线程1] -->|尝试获取锁| M{Monitor锁状态}
    M -->|空闲| T1O[线程1获得锁<br>成为Owner]
    M -->|被占用| T1Q[线程1加入<br>Entry Set队列]
    T1O -->|调用wait| T1W[线程1释放锁<br>进入Wait Set]
    T1O -->|执行完毕| M2[释放锁<br>通知Entry Set]
    M2 -->|唤醒| T2[线程2<br>竞争获取锁]
    T1W -->|其他线程notify| M3[线程1回到<br>Entry Set]
    end
```
...
```mermaid
flowchart TD
    A[初始状态：无锁] -->|第一个线程获取锁| B[偏向锁 101]
    B -->|其他线程尝试获取| C[偏向锁撤销]
    C --> D[轻量级锁 00]
    D -->|CAS失败或自旋超时| E[重量级锁 10]
    
    subgraph 对象头Mark Word状态
    A_Mark[01: 无锁]
    B_Mark[101: 偏向锁]
    D_Mark[00: 轻量级锁]
    E_Mark[10: 重量级锁]
